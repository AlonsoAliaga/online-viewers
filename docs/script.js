...
function updateViewers() {
    fetch('https://alonsoapi.discloud.app/checking-total').catch(e=>{
        ...
        onlineDiv.innerHTML = `<div class="siteoptions"><span>🔴</span> <span><a title="${pageData.description}" href="https://alonsoaliaga.com/donate" target="_blank">🚫 What are you doing here? 🚫</a> 🠊 ${onlineString}</span></div>`
    }).then(res => res.json())
    .then(onlineTotalData => {
        if(typeof onlineTotalData.error != "undefined") {
            onlineDiv.innerHTML = `<div class="siteoptions"><span>🔴</span> <span><a title="${pageData.description}" href="https://alonsoaliaga.com/donate" target="_blank">🚫 What are you doing here? 🚫</a> 🠊 ${onlineString}</span></div>`;
            document.body.innerHTML = `<div class="siteoptions"><span>🔴</span> <span><a title="${pageData.description}" href="https://alonsoaliaga.com/donate" target="_blank">🚫 What are you doing here? 🚫</a> 🠊 ${onlineString}</span></div>`;
            return;
        }
        let dataArray = []
        let unknownArray = []
